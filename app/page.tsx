...
import { ArrowRight, ArrowUpRight, Aperture, CalendarDays, Camera, Clapperboard, Film, MapPin, MonitorPlay, Plane, Radio, Sparkles } from "lucide-react";
import Reveal from "@/components/Reveal";
import { services, packages, whatsapp, portfolioConfig } from "@/lib/data";
...
export default function Home() {
  const featuredProjects = portfolioConfig.projects.concat([
    { id:"p2", title:"Pre-Wedding at Ganga Ghat", category:"Pre-Wedding", type:"video" as const, mediaUrl:"", videoLink:"" },
    { id:"p3", title:"Grand Destination Wedding", category:"Cinematography", type:"video" as const, mediaUrl:"", videoLink:"" },
    { id:"p4", title:"Live Event Production", category:"Events", type:"video" as const, mediaUrl:"", videoLink:"" }
  ]);
  return <main>
    <section className="hero premium-hero">
      <div className="hero-image" /><div className="hero-shade" /><div className="hero-grain" />
      <div className="hero-content">
        ...
        <div className="actions"><Link className="btn gold" href="/portfolio"><span className="play-dot">▶</span> Watch Showreel</Link><Link className="btn glass" href="/portfolio">Our Work <ArrowRight size={16}/></Link></div>
        <div className="hero-stats"><div><strong>500+</strong><span>Happy Clients</span></div><div><strong>1000+</strong><span>Events Covered</span></div><div><strong>5+</strong><span>Years of Excellence</span></div></div>
      </div>
      <div className="hero-script">Frames Beyond<br/><em>Time</em></div><div className="hero-location"><MapPin size={13}/> Varanasi, U.P. India</div>
    </section>
    ...
    <section className="section services-section"><div className="service-grid premium-service-grid">{services.map((service, i) => { const I = icons[i % icons.length]; return <Reveal key={service.id}><Link href={`/services/${service.id}`} className="service-card image-card" style={{"--card-image": `url(${serviceImages[i]})`} as CSSProperties}><div className="card-image"/><div className="card-shade"/><span className="service-index">{String(i + 1).padStart(2, "0")}</span><I className="service-icon"/><div className="service-copy"><h3>{service.title}</h3><p>{service.description}</p></div><span className="circle-arrow"><ArrowUpRight size={17}/></span></Link></Reveal>; })}</div></section>
